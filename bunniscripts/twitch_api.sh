...
[[ ! -d "$cfg" ]] && mkdir "$cfg"
dir=$cfg # need to replace all dir by cfg, but for now it's ok

followings="$dir/twitch_following.txt"

infinity='\u221e'
version="1.1"

usage="USAGE\n\t$0 [-f/-h/-v] [-c/-g/-s/-u/-vi/-w <arg>]\nARGUMENTS"
...
usage="$usage\n\t[-u/--user]\n\t\tGet info about user: id, names, description..."
usage="$usage\n\t[-/--]\n\t\tDescription"
usage=`echo $usage | head -n -2` # this is so we don't print the template at the end

client_id=`cut -d$'\n' -f1 $dir/.credentials.txt`
oauth_token=`cut -d$'\n' -f2 $dir/.credentials.txt`
